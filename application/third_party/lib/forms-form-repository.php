...
class FormRepository extends RedBean_Driver_PDO{
	...
	public function ValidEntryExists($entryId){
		$query = "SELECT Count(*) FROM questions WHERE EntryID='{$entryId}'";
		$result = parent::GetCol($query);
		$count = $result[0];
		return $count > 0;
	}
	 ...
	 {
	    $values = R::getAll(sprintf("SELECT * FROM questions WHERE EntryId='{$entryId}' AND FieldId={$fieldId}"));
		if(count($values) > 0)
		{
		  return $values[0];
		}
	...
	{
	   parent::Execute("DELETE FROM questions WHERE FieldID={$id}");
	...
	public function InsertFieldValue($field, $value, $entryId, $userId = "")
    {
        if (!Utils::isExcludedField($field->FieldType))
        {
			...
			$query="INSERT INTO questions (fieldId, entryId, `Value`, DateAdded) VALUES {$valueString}";
			parent::Execute($query);
        }
    }
	...
	public function ClearExistingValues($entryId, $fieldArray){
	    ...
	    if (!isNullOrEmpty($fieldIdString)){			
			...
			$query = "DELETE FROM questions WHERE EntryId='{$entryId}' AND FieldId IN ({$fieldIdString})";
	...
	{	 
	   //delete submissions 
	   $submissionsQuery = "DELETE questions  FROM questions INNER JOIN form_form_fields ON form_form_fields.fieldId = questions.fieldId WHERE form_form_fields.formId='{$formId}'";
	...
	public function GetRegistrantsByForm($formId, $entryId=null)
	{
		...
		$query = "SELECT questions.*, form_fields.FieldType, form_fields.Label, form_fields.Order, form_fields.DomId FROM form_fields " . 
			     "LEFT JOIN questions ON form_fields.ID =questions.fieldId " . 
				 "INNER JOIN form_form_fields ON form_fields.ID=form_form_fields.fieldId " . 
				 "WHERE form_form_fields.formId='{$formId}' AND questions.EntryId IS NOT NULL " . $entryIdFilter .  				 
				 "ORDER BY form_fields.Order DESC, questions.DateAdded DESC";
	...
	public function deleteEntry($entryId){
		...
		$query = "DELETE FROM questions WHERE EntryId='{$entryId}'";
		parent::Execute(sprintf($query));
	}
